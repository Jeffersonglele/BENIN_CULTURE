...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <table id="mediasTable" class="table table-bordered table-striped display" style="width:100%">
                    <thead>
                        <tr>
                            <th>Médias</th>
                            <th style="width: 120px">Type de médias</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $media->titre }}</td>
                            <td>{{ $media->typeMedia->nom_type_media ?? 'Non défini' }}</td>
                            <td class="text-center actions-cell">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('author.medias.show', $media->id) }}" 
                                       class="btn btn-sm btn-warning btn-action" 
                                       data-bs-toggle="tooltip" 
                                       title="Voir les détails">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="{{ route('author.medias.edit', $media->id) }}" 
                                       class="btn btn-sm btn-primary btn-action" 
                                       data-bs-toggle="tooltip" 
                                       title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('author.medias.destroy', $media->id) }}" 
                                          method="POST" 
                                          class="delete-form" 
                ...
                <div class="btn-add-container float-end">
                    <a href="{{ route('author.medias.create') }}" class="btn btn-primary">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var table = document.getElementById('mediasTable');
        if (table) {
            ...
            console.error('La table avec l\'ID "mediasTable" n\'a pas été trouvée');
        }
    });
</script>
@endpush