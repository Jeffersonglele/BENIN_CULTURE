...
<style>
    ...
    /* Largeur spécifique pour le conteneur du tableau */
    .table-responsive {
        width: 100%;
        overflow-x: auto;
    }
...
<div class="row">
            ...
            <div class="table-responsive">
                <table id="contenusTable" class="table table-bordered table-striped display" style="width:100%">
                    <thead>
                        <tr>
                            <th>Titre</th>
                            <th>Statut</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $contenu->region->nom_region ?? 'Non défini' }}</td>
                            <td>{{ $contenu->langue->nom_langue ?? 'Non défini' }} </td>
                            <td>{{ $contenu->auteur->nom ?? 'Non défini' }} {{ $contenu->auteur->prenom ?? 'Non défini' }}</td>
                            <td>{{ $contenu->typeContenu->nom_type_contenu ?? 'Non défini' }}</td>
                            <td class="text-center actions-cell">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('author.contenus.show', $contenu->id) }}" 
                                       class="btn btn-sm btn-warning btn-action" 
                                       data-bs-toggle="tooltip" 
                                       title="Voir les détails">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="{{ route('author.contenus.edit', $contenu->id) }}" 
                                       class="btn btn-sm btn-primary btn-action" 
                                       data-bs-toggle="tooltip" 
                                       title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('author.contenus.destroy', $contenu->id) }}" 
                                          method="POST" 
                                          class="delete-form" 
                                          onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette contenu ?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                ...
                <div class="btn-add-container float-end">
                    <a href="{{ route('author.contenus.create') }}" class="btn btn-primary">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var table = document.getElementById('contenusTable');
        if (table) {
            ...
            console.error('La table avec l\'ID "contenusTable" n\'a pas été trouvée');
        }
    });
</script>
@endpush